...
export function groupBillsByHalf(bills: BillViewModel[]): { firstHalfBills: BillViewModel[], secondHalfBills: BillViewModel[] } {
  return {
    firstHalfBills: bills.filter(b => (parseInt(b.dueDay, 10) || 1) <= 15),
    secondHalfBills: bills.filter(b => (parseInt(b.dueDay, 10) || 1) > 15),
  };
}
