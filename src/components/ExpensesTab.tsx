...
import { Camera, ScanLine, Plus, Receipt, Trash2, CheckCircle2 } from "lucide-react";
...
import { useExpenseScanner } from "../hooks/useExpenseScanner";
...
export const ExpensesTab: React.FC<ExpensesTabProps> = ({ globalData, setGlobalData, showToast }) => {
  ...
  const { isScanning, fileInputRef, handleCapture, stopScanning } = useExpenseScanner({
    showToast,
    globalData,
    onScanStart: () => setShowForm(true),
    onScanComplete: (result) => setForm(prev => ({ ...prev, ...result })),
  });
    ...
    handleSaveExpense(
      ...
      () =>
        setForm({
          ...
          wallet: getDefaultExpenseWalletId(
            "Food & Dining",
            globalData.settings,
            globalData.wallets
          ),
      ...
      <div className="bg-surface-low border border-inverse/[0.08] rounded-2xl p-5 shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center text-center space-y-3 py-4">
          ...
          <div className="flex gap-2 w-full max-w-xs mt-4">
            ...
            <button onClick={() => { setShowForm(true); stopScanning(); }} className="flex-1 bg-surface-high hover:bg-inverse/[0.06] border border-inverse/[0.06] text-secondary font-semibold py-3 rounded-xl text-xs flex items-center justify-center gap-2 transition shadow-md">
              <Plus size={14} /> Manual
            </button>
          </div>
        </div>
