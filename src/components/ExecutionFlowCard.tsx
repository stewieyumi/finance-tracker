...
export const ExecutionFlowCard: React.FC<ExecutionFlowCardProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
      ...
      <div className="bg-surface-low border border-inverse/[0.08] rounded-2xl p-4 sm:p-5 shadow-xl flex flex-col justify-between">
        <div>
          ...
          <div className="space-y-2 text-xs">
            <div className="flex items-center justify-between p-2.5 rounded-xl bg-surface border border-inverse/[0.04]">
              <span className="text-muted font-medium no-privacy-blur">Top Priority Unpaid</span>
            ...
            <div className="flex items-center justify-between p-2.5 rounded-xl bg-surface border border-inverse/[0.04]">
              <span className="text-muted font-medium no-privacy-blur">Total Commitments Left</span>
      ...
      <div className="bg-surface-low border border-inverse/[0.08] rounded-2xl p-4 sm:p-5 shadow-xl flex flex-col justify-between">
        <div>
          <div className="flex items-center justify-between mb-3.5">
            ...
            <button onClick={onConfigureBaselines} className="text-faint hover:text-amber-400 transition" title="Configure Baselines & Routing">
              <Settings size={13} />
            </button>
          </div>
          ...
          <div className="space-y-1.5 text-xs">
            {Object.entries(paydayAllocations).filter(([_, amount]) => amount > 0).map(([walletKey, amount]) => (
              <div key={walletKey} className="flex items-center justify-between py-1 border-b border-inverse/[0.03]">
                <span className="privacy-blur text-muted">{customWallets?.find(w => w.id === walletKey)?.label || walletKey.charAt(0).toUpperCase() + walletKey.slice(1)}</span>
          ...
          <div className="mt-3 flex items-center justify-between bg-fill/40 border border-strong/80 p-2.5 rounded-xl">
             <div className="flex items-center gap-2">
        ...
        <button
          ...
          disabled={disabled}
          className="mt-3 w-full bg-blue-600 hover:bg-blue-500 active:bg-blue-700 disabled:bg-fill-strong disabled:text-faint disabled:cursor-not-allowed text-white font-semibold text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 transition shadow-lg shadow-blue-600/20 no-privacy-blur"
        >
