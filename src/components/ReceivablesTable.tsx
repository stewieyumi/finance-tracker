...
export const ReceivablesTable: React.FC<ReceivablesTableProps> = React.memo(({
  ...
  const [newReceivable, setNewReceivable] = useState({
    name: "", amount: "", category: "Shoot" as ReceivableCategory, frequency: "By Date" as ReceivableFrequency,
    biMonthlyDays: "15th & 30th", monthlyDay: "15", date: "", wallet: "maya"
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAddReceivable({
      name: newReceivable.name, amount: parseFloat(newReceivable.amount) || 0, category: newReceivable.category,
      frequency: newReceivable.frequency, biMonthlyDays: newReceivable.frequency === "Bi-monthly" ? newReceivable.biMonthlyDays : undefined,
      monthlyDay: newReceivable.frequency === "Monthly" ? newReceivable.monthlyDay : undefined, date: newReceivable.frequency === "By Date" ? newReceivable.date : undefined,
      wallet: newReceivable.wallet
    });
      ...
      <div key={rec.id} className={`p-3 rounded-xl border transition-all ${rec.collected ? "bg-zinc-950/40 border-zinc-900/60 opacity-50" : "bg-[#14141a] border-zinc-800/80 shadow-sm"}`}>
        {isEditing ? (
          <div className="space-y-2">
            <input type="text" value={editForm.name || ""} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} placeholder="Name" className="bg-[#0b0b0d] border border-zinc-700 rounded-lg px-2.5 py-1.5 text-white text-xs w-full outline-none" />
            <div className="grid grid-cols-2 gap-2">
              <input type="number" inputMode="decimal" step="0.01" value={editForm.amount ?? ""} onChange={(e) => setEditForm({ ...editForm, amount: parseFloat(e.target.value) || 0 })} placeholder="Total Amount" className="bg-[#0b0b0d] border border-zinc-700 rounded-lg px-2 py-1.5 text-white text-xs font-mono outline-none" />
              <select value={editForm.category || "Salary"} onChange={(e) => setEditForm({ ...editForm, category: e.target.value as ReceivableCategory })} className="bg-[#0b0b0d] border border-zinc-700 rounded-lg px-2 py-1.5 text-white text-xs">
                {categoriesList.map(cat => <option key={cat} value={cat}>{cat}</option>)}
              </select>
              <select value={editForm.wallet || "maya"} onChange={(e) => setEditForm({ ...editForm, wallet: e.target.value })} className="bg-[#0b0b0d] border border-zinc-700 rounded-lg px-2 py-1.5 text-white text-[10px] font-semibold uppercase outline-none col-span-2">
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <button onClick={() => onToggleStatus(rec)} aria-label={`Toggle collection for ${rec.name}`} className="shrink-0 focus:outline-none">
                  {rec.collected ? (
                    <span className="w-4 h-4 rounded-full bg-emerald-950/70 border border-emerald-500/50 text-emerald-400 flex items-center justify-center"><Check size={9} className="stroke-[3]" /></span>
                  ) : (
                    <span className="w-4 h-4 rounded-full bg-amber-950/40 border border-amber-500/40 text-amber-400 flex items-center justify-center"><Hourglass size={8} /></span>
                  )}
                </button>
      ...
      <tr key={rec.id} className={`group transition-all duration-150 ${rec.collected ? "opacity-45" : "hover:bg-white/[0.02]"}`}>
        <td className="py-2.5 px-3 whitespace-nowrap">
          <button onClick={() => onToggleStatus(rec)} aria-label={`Toggle collection for ${rec.name}`} className="flex items-center gap-1.5 focus:outline-none">
            {rec.collected ? (
              <span className="flex items-center justify-center gap-1 w-[82px] text-emerald-400 text-[10px] font-semibold bg-emerald-950/40 px-2 py-0.5 rounded-lg border border-emerald-600/30"><Check size={10} className="stroke-[3]" /> Received</span>
            ) : isPartial ? (
              <span className="flex items-center justify-center gap-1 w-[82px] text-cyan-400 text-[10px] font-semibold bg-cyan-950/40 px-2 py-0.5 rounded-lg border border-cyan-600/30"><CreditCard size={9} /> Partial</span>
            ) : (
              <span className="flex items-center justify-center gap-1 w-[82px] text-amber-400 text-[10px] font-medium bg-amber-950/30 px-2 py-0.5 rounded-lg border border-amber-800/30"><Hourglass size={8} /> Pending</span>
            )}
          </button>
        </td>
        ...
        <td className="py-2.5 px-3 text-zinc-200 font-medium">
          {isEditing ? (
            <input type="text" value={editForm.name || ""} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="bg-[#0b0b0d] border border-zinc-700 rounded-md px-2 py-1 text-white text-xs w-full focus:ring-1 focus:ring-emerald-500 outline-none" />
          ) : <span>{rec.name}</span>}
        </td>
        ...
        <td className={`py-2.5 px-3 text-right font-mono font-semibold whitespace-nowrap ${rec.collected ? "text-emerald-400" : "text-zinc-100"}`}>
          {isEditing ? (
            <input type="number" inputMode="decimal" step="0.01" value={editForm.amount ?? ""} onChange={(e) => setEditForm({ ...editForm, amount: parseFloat(e.target.value) || 0 })} className="bg-[#0b0b0d] border border-zinc-700 rounded-md px-2 py-1 text-white text-xs w-20 text-right focus:ring-1 focus:ring-emerald-500 outline-none" />
          ) : (
            <div>
        ...
        <td className="py-2.5 px-3 text-center whitespace-nowrap">
          {isEditing ? (
            <div className="flex flex-col gap-1.5">
              <select value={editForm.category || "Shoot"} onChange={(e) => setEditForm({ ...editForm, category: e.target.value as ReceivableCategory })} className="bg-[#0b0b0d] border border-zinc-700 rounded-md px-1.5 py-1 text-white text-xs">
                {categoriesList.map(cat => <option key={cat} value={cat}>{cat}</option>)}
              </select>
              <select value={editForm.wallet || "maya"} onChange={(e) => setEditForm({ ...editForm, wallet: e.target.value })} className="bg-[#0b0b0d] border border-zinc-700 rounded-md px-1.5 py-1 text-white text-[10px] font-semibold uppercase outline-none">
                {allWallets.map(w => <option key={w.id} value={w.id}>{w.label}</option>)}
              </select>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-1">
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-md ${rec.category === "Salary" ? "bg-cyan-950/70 text-cyan-300 border border-cyan-800/40" : rec.category === "Shoot" ? "bg-amber-950/70 text-amber-300 border border-amber-800/40" : rec.category === "Edit" ? "bg-purple-950/70 text-purple-300 border border-purple-800/40" : "bg-zinc-800 text-zinc-300 border border-zinc-700/40"}`}>
                {rec.category || "Other"}
              </span>
              {rec.wallet && <span className="bg-zinc-800/80 text-zinc-300 border border-zinc-700/40 px-1.5 py-0.5 rounded font-semibold tracking-wider text-[9px] uppercase">{allWallets.find(w => w.id === rec.wallet)?.label || rec.wallet}</span>}
            </div>
        ...
        <td className="py-2.5 px-3 text-center whitespace-nowrap">
          {isEditing ? (
            <div className="flex items-center justify-center gap-1">
              <select value={editForm.frequency || "By Date"} onChange={(e) => setEditForm({ ...editForm, frequency: e.target.value as ReceivableFrequency })} className="bg-[#0b0b0d] border border-zinc-700 rounded-md px-1 py-1 text-white text-[11px]">
                <option value="By Date">Date</option><option value="Monthly">Monthly</option><option value="Bi-monthly">15/30</option>
              </select>
              {editForm.frequency === "Monthly" && (
                <select value={editForm.monthlyDay || "15"} onChange={(e) => setEditForm({ ...editForm, monthlyDay: e.target.value })} className="bg-[#0b0b0d] border border-zinc-700 rounded-md px-1 py-1 text-white text-[10px]">
                  {Array.from({length: 31}, (_, i) => i + 1).map(d => <option key={d} value={String(d)}>Day {d}</option>)}
                </select>
              )}
              {editForm.frequency === "By Date" && (
                <input type="date" value={editForm.date || ""} onChange={(e) => setEditForm({ ...editForm, date: e.target.value })} className="bg-[#0b0b0d] border border-zinc-700 rounded-md px-1.5 py-1 text-white text-[10px] min-w-[110px] flex-1" />
              )}
            </div>
          ) : (
        ...
        <td className="py-2.5 px-3 text-right whitespace-nowrap relative">
          <div className="inline-flex items-center gap-1 justify-end">
            {isEditing ? (
              <>
                <button onClick={handleCancelEdit} className="p-1 text-zinc-400 hover:text-white bg-zinc-800 rounded-md text-[10px] transition"><X size={11} /></button>
                <button onClick={() => onSaveEdit("receivables")} className="p-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-md text-[10px] transition"><Save size={11} /></button>
                <button onClick={() => onDeleteReceivable(rec.id)} className="p-1 text-zinc-400 hover:text-rose-400 bg-zinc-800 rounded-md text-[10px] transition"><Trash2 size={11} /></button>
              </>
            ) : (
              <>
                {!rec.collected && (
    ...
    <div className="bg-[#101014] border border-white/[0.08] rounded-2xl p-4 sm:p-5 shadow-xl w-full">
        ...
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="text-zinc-400 border-b border-white/[0.06] text-[11px]">
              ...
              <th className="py-2.5 px-3 font-semibold text-center w-[90px]">Category & Route</th>
      ...
      <form onSubmit={handleSubmit} className="mt-3.5 pt-3 border-t border-white/[0.06] flex flex-col lg:flex-row items-stretch lg:items-center gap-2">
        <div className="flex items-center gap-1.5 w-full lg:flex-1">
          <input type="text" placeholder="New inflow name..." value={newReceivable.name} onChange={(e) => setNewReceivable({ ...newReceivable, name: e.target.value })} className="bg-[#0b0b0d] border border-zinc-800 focus:border-emerald-500/60 rounded-xl px-3 py-1.5 text-xs text-white placeholder-zinc-500 outline-none w-full flex-1" />
          <input type="number" inputMode="decimal" step="0.01" placeholder="₱ Amount" value={newReceivable.amount} onChange={(e) => setNewReceivable({ ...newReceivable, amount: e.target.value })} className="bg-[#0b0b0d] border border-zinc-800 focus:border-emerald-500/60 rounded-xl px-2.5 py-1.5 text-xs text-white placeholder-zinc-500 font-mono outline-none w-28 shrink-0" />
        </div>

        <div className="flex flex-wrap lg:flex-nowrap items-center gap-1.5 w-full lg:w-auto">
          <select value={newReceivable.category} onChange={(e) => setNewReceivable({ ...newReceivable, category: e.target.value as ReceivableCategory })} className="bg-[#0b0b0d] border border-zinc-800 rounded-xl px-1 sm:px-2 py-1.5 text-[11px] sm:text-xs text-white outline-none flex-1 lg:w-auto truncate min-w-[80px]">
            {categoriesList.map(cat => <option key={cat} value={cat}>{cat}</option>)}
          </select>

          <select value={newReceivable.wallet} onChange={(e) => setNewReceivable({ ...newReceivable, wallet: e.target.value })} className="bg-[#0b0b0d] border border-zinc-800 rounded-xl px-1 sm:px-2 py-1.5 text-[11px] sm:text-xs text-blue-300 font-semibold uppercase outline-none flex-1 lg:w-auto min-w-[80px]">
            {allWallets.map(w => <option key={w.id} value={w.id}>{w.label}</option>)}
          </select>

          <select value={newReceivable.frequency} onChange={(e) => setNewReceivable({ ...newReceivable, frequency: e.target.value as ReceivableFrequency })} className="bg-[#0b0b0d] border border-zinc-800 rounded-xl px-1 sm:px-2 py-1.5 text-[11px] sm:text-xs text-white outline-none flex-1 lg:w-auto min-w-[85px]">
            <option value="By Date">By Date</option><option value="Monthly">Monthly</option><option value="Bi-monthly">15th/30th</option>
          </select>

          {newReceivable.frequency === "Monthly" && (
            <select value={newReceivable.monthlyDay} onChange={(e) => setNewReceivable({ ...newReceivable, monthlyDay: e.target.value })} className="bg-[#0b0b0d] border border-zinc-800 rounded-xl px-1 sm:px-2 py-1.5 text-[11px] sm:text-xs text-white outline-none flex-1 lg:w-auto min-w-[80px]">
              {Array.from({length: 31}, (_, i) => i + 1).map(d => <option key={d} value={String(d)}>Day {d}</option>)}
            </select>
          )}

          {newReceivable.frequency === "By Date" && (
            <input type="date" value={newReceivable.date} onChange={(e) => setNewReceivable({ ...newReceivable, date: e.target.value })} className="bg-[#0b0b0d] border border-zinc-800 rounded-xl px-2 py-1.5 text-[11px] sm:text-xs text-white outline-none flex-1 lg:w-32 min-w-[115px]" />
          )}
        </div>

        <button type="submit" className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-semibold flex items-center justify-center gap-1 shadow-lg shadow-emerald-950/40 transition shrink-0 w-full lg:w-auto">
          <Plus size={13} /><span>Add Inflow</span>
        </button>
      </form>
    </div>
  );
