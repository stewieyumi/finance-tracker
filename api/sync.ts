...
function isValidPayload(payload: unknown): payload is SyncPayload {
  if (!payload || typeof payload !== "object") return false;
  const data = payload as SyncPayload;
  if (!isValidUpdatedAt(data.updatedAt)) return false;
  if (!data.wallets || typeof data.wallets !== "object") return false;
  if (!data.library || typeof data.library !== "object") return false;
  return true;
}
...
export default async function handler(req: VercelApiRequest, res: VercelApiResponse) {
  ...
  if (!process.env.KV_REST_API_URL || !process.env.KV_REST_API_TOKEN) {
    return res.status(500).json({ error: "Server configuration error: Missing Redis credentials." });
  }
  ...
  try {
    ...
    if (req.method === "PUT" || req.method === "POST") {
      ...
      if (!isValidPayload(payload)) {
        return res.status(400).json({ error: "Invalid sync payload: updatedAt, wallets, and library are required." });
      }

      const existing = await redis.get<SyncPayload>(USER_REDIS_KEY);
      const existingUpdatedAt = existing && isValidUpdatedAt(existing.updatedAt) ? existing.updatedAt : null;

      if (existingUpdatedAt !== null && payload.updatedAt <= existingUpdatedAt) {
        return res.status(200).json({ success: true, accepted: false, cloudUpdatedAt: existingUpdatedAt, data: existing });
      }

      await redis.set(USER_REDIS_KEY, payload);
      return res.status(200).json({ success: true, accepted: true, updatedAt: payload.updatedAt });
    }
